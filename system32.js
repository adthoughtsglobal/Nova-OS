    ...
    async setById(id, content) {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                request.onsuccess = async () => {
                    try {
                        const result = request.result || { contentpool: {}, memory: {} };
                        const compressedContent = compressString(content);
                        const encryptedContent = await encryptData(cryptoKeyCache, compressedContent);
                        ...
                        console.error("Encryption error in setById:", error);
                        reject(error);
                    }
                };
